...
import { readState, writeState, StackState, ServiceState, getStack } from "../state";
...
import { classifyServices, CandidateHashes } from "./classify";
...
export async function deploy(options: DeployOptions): Promise<void> {
  ...
  try {
    ...
    // Retrieve existing stack state for comparison
    const existingStackState = getStack(state, config.stack.name);
    const previousEnvHash = existingStackState?.env_hash ?? null;
    const envHashChanged = currentEnvHash !== previousEnvHash;
